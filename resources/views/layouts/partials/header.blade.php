...
<header class="main-header no-print">
  <a href="{{route('home')}}" class="logo">
    ...
    <span class="logo-lg">
      ...
      @endif
      <i class="fa fa-circle text-success" id="online_indicator"></i></span>
  ...
  <nav class="navbar navbar-static-top" role="navigation">
    ...
    <div class="navbar-custom-menu">
      ...
      <a href="{{action('SellPosController@create')}}" title="@lang('sale.pos_sale')" data-toggle="tooltip"
        data-placement="bottom" class="btn btn-rounded pull-left m-8 btn-sm mt-10 btn-secondary-boxity">
      ...
      <div class="btn-group">
        <button id="header_shortcut_dropdown" type="button"
          class="btn btn-secondary-boxity dropdown-toggle btn-rounded pull-left m-8 btn-sm mt-10" data-toggle="dropdown"
      ...
      <button id="btnCalculator" title="@lang('lang_v1.calculator')" type="button"
        class="btn btn-secondary-boxity btn-rounded pull-left m-8 btn-sm mt-10 popover-default hidden-xs"
      ...
      <button type="button" id="register_details" title="{{ __('cash_register.register_details') }}"
        ...
        class="btn btn-secondary-boxity btn-rounded pull-left m-8 btn-sm mt-10 btn-modal"
        data-container=".register_details_modal" data-href="{{ action('CashRegisterController@getRegisterDetails')}}">
        <strong><i class="fa fa-briefcase fa-lg" aria-hidden="true"></i></strong>
      </button>
      @endcan
      @can('close_cash_register')
      <button type="button" id="close_register" title="{{ __('cash_register.close_register') }}" data-toggle="tooltip"
        data-placement="bottom" class="btn btn-danger btn-rounded pull-left m-8 btn-sm mt-10 btn-modal"
      ...
      <button type="button" id="view_todays_profit" title="{{ __('home.todays_profit') }}" data-toggle="tooltip"
        data-placement="bottom" class="btn btn-secondary-boxity btn-rounded pull-left m-8 btn-sm mt-10">
        <strong><i class="fas fa-money-bill-alt fa-lg"></i></strong>
      </button>
      @endcan

      <div class="m-8 pull-left mt-15 hidden-xs" style="color: #fff;">
        <strong>{{ Carbon::now()->format('d-m-Y h:m A') }}</strong></div>
      ...
      <ul class="nav navbar-nav">
        ...
        <li class="dropdown user user-menu">
          ...
          <ul class="dropdown-menu">
            ...
            <li class="user-header" style="padding: 1rem 3rem;">
              @if(!empty(Session::get('business.logo')))
              <img src="{{ asset( 'uploads/business_logos/' . Session::get('business.logo') ) }}" alt="Logo">
              @endif
              <p>
            ...
            <li class="user-footer">
              <div class="pull-left">
                <a href="{{action('UserController@getProfile')}}"
                  class="btn btn-default btn-rounded">@lang('lang_v1.profile')</a>
              </div>
              <div class="pull-right">
                <a href="{{action('Auth\LoginController@logout')}}"
                  class="btn btn-default btn-rounded">@lang('lang_v1.sign_out')</a>
              </div>
            </li>
          </ul>
        </li>
